...
fn is_in_check() {
  ...
  */
  ...
  // Case 11
  test!("4r3/8/8/4K2r/8/8/8/8 w - - 0 1", White, true);
}
...
fn is_checkmate() {
  ...
  assert!(Board::from_fen("K6r/7r/8/8/8/8/8/8 w - - 0 1")
    .expect("Failed to load FEN")
    .is_in_checkmate(Color::White));

  // Case 2: Anderssen's mate
  assert!(Board::from_fen("6kR/6P1/5K2/8/8/8/8/8 w - - 0 1")
    .expect("Failed to load FEN")
    .is_in_checkmate(Color::Black));

  // Case 3: Arabian mate
  assert!(Board::from_fen("7k/7R/5N2/8/8/8/8/8 w - - 0 1")
    .expect("Failed to load FEN")
    .is_in_checkmate(Color::Black));

  // Case 4: Backrank mate
  assert!(Board::from_fen("3R2k1/5ppp/8/8/8/8/8/8 w - - 0 1")
    .expect("Failed to load FEN")
    .is_in_checkmate(Color::Black));

  // Case 4: Balestra mate
  assert!(Board::from_fen("6k1/8/4B2Q/8/8/8/8/8 w - - 0 1")
    .expect("Failed to load FEN")
    .is_in_checkmate(Color::Black));

  // Case 5: Bishop and knight mate
  assert!(Board::from_fen("7k/8/5BKN/8/8/8/8/8 w - - 0 1")
    .expect("Failed to load FEN")
    .is_in_checkmate(Color::Black));

  // Case 6: Blackburne's mate
  assert!(Board::from_fen("5rk1/7B/8/6N1/8/8/1B6/8 w - - 0 1")
    .expect("Failed to load FEN")
    .is_in_checkmate(Color::Black));

  // Case N: Not a mate
  assert!(!Board::from_fen("K6r/7r/6N1/8/8/8/8/8 w - - 0 1")
    .expect("Failed to load FEN")
    .is_in_checkmate(Color::White));
}
