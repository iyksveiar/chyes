/* OLD FILE, USING FOR COPYING CODE OVER TO NEW FILE ^_^ */
...
impl Board {
  ...
  pub fn place_piece(
    ...
    let old_piece = self.get_piece(coord).expect("Invalid coordinate");
  ...
  pub fn modify_sq(
    ...
    let old_piece = self.get_piece(coord).expect("Could not get piece");
  ...
  pub fn apply_move(
    ...
    let piece = self.get_piece(starting).expect("Could not get piece");

    let captured_piece = self.place_piece(piece, ending).expect("Could not place piece");

    self.place_piece(piece!(Empty, White), starting);
  ...
  pub fn filter_check_moves(
    ...
    let piece = self.get_piece(coord).expect("Can't get moves of a piece");
